...
mod test {
    ...
    fn test_authority_key_exchange() {
        let config: Result<Config, _> = toml::from_str(r#"
        network-id = "secret society"
        key = { value = "12345678901234567890123456789012" }
        [auth]
        ca = { value = "generated by code" }
    ...
    "#);
        ...
        let pub_key = config.get_public_key();

        if let AuthConfig::CertificateAuthorityConfig(ref mut auth) = config.auth {
            ...
            auth.signature = FileOrValue::with_value(Bytes::from(key_pair.sign(&pub_key).as_ref()));
            auth.ca = FileOrValue::with_value(Bytes::from(key_pair.public_key_bytes()));
        } else {
            assert!(false);
        }
        ...
        assert_eq!(&message.public_key, &config.get_public_key());
